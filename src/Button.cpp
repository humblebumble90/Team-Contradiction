...
Button::Button(std::string buttonName, GameObjectType type, glm::vec2 position, bool isCentered): m_name(buttonName), m_isCentered(isCentered), m_alpha(255)
{
	changeTexture(buttonName);
	glm::vec2 size = TextureManager::Instance()->getTextureSize(name);
	setWidth(size.x);
	setHeight(size.y);
